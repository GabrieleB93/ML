...
def train(X, Y, batch_size=914, lr=0.001, n_layers=5, hidden_size=100, epochs=5000, activation='relu'):
    model = Sequential()
    ...
    for tr_index, ts_index in rkfold.split(X, Y):
        ...
        hist = model.fit(x_train, y_train, shuffle=True, epochs=epochs, verbose=0, batch_size=batch_size, validation_data=(x_test, y_test))
